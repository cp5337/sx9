...
export default function Missions() {
  ...
  const [creatingLinearIssue, setCreatingLinearIssue] = useState(false);
  ...
  const loadLinearTeams = async () => {
    try {
      const teams =
        await invoke<{ id: string; name: string; key: string }[]>(
          "list_linear_teams"
        );
  ...
  const createLinearIssue = async (missionId: string) => {
    if (!selectedTeam) {
      alert("Please select a Linear team");
      return;
    }

    setCreatingLinearIssue(true);
    try {
      ...
      alert(`✅ Linear issue created: ${issue.identifier}`);
      loadMissions();
    } catch (error) {
      alert(`❌ Failed to create Linear issue: ${error}`);
    } finally {
      setCreatingLinearIssue(false);
    }
  };
                ...
                {selected.linear_issue_url ? (
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() => createLinearIssue(selected.id)}
                        disabled={creatingLinearIssue}
                        className="px-3 py-1 bg-emerald-600 hover:bg-emerald-700 disabled:bg-zinc-700 disabled:text-zinc-500 rounded text-sm font-medium transition-colors"
                      >
                        {creatingLinearIssue ? "Creating..." : "Create"}
                      </button>
                    </div>
                  </div>
                )}
