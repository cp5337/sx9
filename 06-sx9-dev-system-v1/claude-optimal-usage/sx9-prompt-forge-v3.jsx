...
export default function SX9PromptForgeV3() {
  // State
  const [promptType, setPromptType] = useState('CUSTOM');
  const [harness, setHarness] = useState('build');
  ...
  useEffect(() => {
    ...
    if (type) {
      const p = type.persona ? PERSONAS[type.persona] : null;
      const h = p ? HARNESSES[p.harness] : HARNESSES.build;
      
      setPersona(type.persona || '');
      if (p) setHarness(p.harness);
    ...
    <div className="min-h-screen bg-zinc-900 text-zinc-100 p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-4 pb-3 border-b border-zinc-700">
          <div>
            <h1 className="text-xl font-bold text-emerald-400">SX9 PROMPT FORGE v3</h1>
            <p className="text-xs text-zinc-500">Linear • Canva • Full Claude Integration</p>
          </div>
          ...
          <div className="bg-zinc-800/50 rounded p-3 border border-zinc-700">
            ...
            <div className="grid grid-cols-5 gap-1">
              {Object.entries(HARNESSES).map(([k, h]) => (
                <button key={k} onClick={() => setHarness(k)}
                  ...
                  <div className="grid grid-cols-4 gap-2 mt-2">
                    {[
                      ...
                      <label key={k} className={`${checkClass} p-2 rounded border ${form[k] ? 'border-emerald-700 bg-emerald-900/20' : 'border-zinc-700'}`}>
                        <input type="checkbox" checked={form[k]} onChange={e => set(k, e.target.checked)} className="w-3 h-3" />
                        <span>{i}</span>
