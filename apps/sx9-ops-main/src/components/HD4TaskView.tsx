...
const HD4TaskView: React.FC<HD4TaskViewProps> = ({
  hd4Action,
  showCLI: externalShowCLI,
  onToggleCLI,
  onTaskSelect,
}) => {
    ...
    try {
      ...
      for (const phaseName of phaseVariations) {
        ...
        if (!fetchError && phaseData && phaseData.length > 0) {
          data = phaseData;
          console.log(
            `✅ HD4TaskView: Loaded ${data.length} tasks for ${hd4Action} phase (matched: ${phaseName})`
          );
          break;
        }
      ...
      if (data.length === 0) {
        ...
        if (!allError && allTasks) {
          ...
          console.log(
            `✅ HD4TaskView: Loaded ${data.length} tasks for ${hd4Action} phase (client-side filter)`
          );
        }
      }
                          ...
                          <div className="mb-2">
                            ...
                            <div className="flex flex-wrap gap-1 mt-1">
                              {task.kali_tools.map((tool, idx) => (
                                <span
                                  key={idx}
                                  className="px-1.5 py-0.5 bg-green-900 text-green-300 rounded text-xxs"
                                >
                                  {tool}
                                </span>
                              ))}
                          ...
                          <div className="mb-2">
                            ...
                            <div className="flex flex-wrap gap-1 mt-1">
                              {task.primitives.map((primitive, idx) => (
                                <span
                                  key={idx}
                                  className="px-1.5 py-0.5 bg-purple-900 text-purple-300 rounded text-xxs"
                                >
                                  {primitive}
                                </span>
                              ))}
