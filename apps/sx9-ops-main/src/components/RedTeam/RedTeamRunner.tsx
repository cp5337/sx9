...
import { CheckCircle, Target, Info, Play, Shield } from 'lucide-react';
...
interface RedTeamRunnerProps {
  phase: 'Hunt' | 'Detect' | 'Disable' | 'Disrupt' | 'Dominate';
  nodeId?: string;
}

const RedTeamRunner: React.FC<RedTeamRunnerProps> = ({ phase, nodeId }) => {
  ...
  const [tests, setTests] = useState<RedTeamTest[]>([
    {
      ...
      command: 'REG ADD "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run" /V "Red Team" /t REG_SZ /F /D "C:\\Path\\RedTeam.exe"',
      status: 'ready',
      cleanupCommand: 'REG DELETE "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run" /V "Red Team" /f'
    },
    {
      id: '2',
  ...
  const getStatusIcon = (status: RedTeamTest['status']) => {
    switch (status) {
      case 'ready':
