...
export interface GlyphRailItem {
  id: string;
  icon: LucideIcon;
  label: string;
  position: "left" | "right";
  statusIndicator?: "connected" | "running" | "disconnected" | "stopped" | "error" | "warning";
  pulse?: boolean;
}
...
export default function GlyphRail({ items, activeId, onItemClick, position }: GlyphRailProps) {
      ...
      >
        ...
        {item.statusIndicator && (
          <div className="absolute top-1 right-1">
            <ServiceStatusIndicator status={item.statusIndicator} size="sm" pulse={item.pulse} />
          </div>
        )}
...
export function SidePanel({ isOpen, position, title, children }: SidePanelProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-dark-border">
        <h3 className="text-sm font-semibold text-gray-900 dark:text-dark-text-primary">{title}</h3>
      </div>
