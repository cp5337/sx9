...
const HD4PhaseContent: React.FC<HD4PhaseContentProps> = ({ phase, view }) => {
  const [activeTab, setActiveTab] = useState<
    "overview" | "playbooks" | "kali" | "redteam" | "mapping" | "tasks"
  >("overview");
    ...
    <div className="h-full flex flex-col bg-gray-900 text-gray-300">
      ...
      <div className="flex-1 overflow-hidden">
          ...
          <div id="hd4-overview-container" className="h-full flex flex-col relative">
            ...
            >
              <div className="flex flex-col items-center">
                ...
                <div className="flex gap-1">
                  <div
                    className={`w-1 h-1 rounded-full ${isResizing ? "bg-white" : "bg-gray-500 group-hover:bg-blue-400"}`}
                  />
                  <div
                    className={`w-1 h-1 rounded-full ${isResizing ? "bg-white" : "bg-gray-500 group-hover:bg-blue-400"}`}
                  />
              ...
              <div className="absolute right-4 text-xs text-gray-400 group-hover:text-blue-400">
                Drag to resize • Map: {Math.round(mapHeight)}% • Tasks:{" "}
                {Math.round(100 - mapHeight)}%
              </div>
            </div>
            ...
            {showCLI ? (
              <div
                ...
                style={{ height: "40%", minHeight: "300px" }}
              >
                <MultiCLI
                  currentHD4Phase={
                    phase.toLowerCase() as "hunt" | "detect" | "disrupt" | "disable" | "dominate"
                  }
                  className="h-full"
              ...
              <div className="flex-1 overflow-hidden flex flex-col" style={{ minHeight: "200px" }}>
                {/* Agent Tabs */}
                ...
                <div className="flex-1 overflow-hidden flex">
                  {(agentView === "chat" || agentView === "split") && (
                    <div
                      className={
                        agentView === "split" ? "w-1/2 border-r border-gray-700" : "w-full"
                      }
                      onDragOver={e => {
                        e.preventDefault();
                        e.stopPropagation();
        ...
        {activeTab === "kali" && (
          <div className="h-full flex flex-col overflow-hidden">
            <div className="flex-1 overflow-y-auto">
              <KaliToolsIntegration activeTask={selectedTask} />
            </div>
            {showCLI && (
              <div
                className="flex-none border-t border-gray-700"
        ...
        {activeTab === "playbooks" && (
          <div className="h-full flex flex-col overflow-hidden">
            <div className="flex-1 overflow-y-auto">
              <Playbooks activeTask={selectedTask} />
            </div>
            {showCLI && (
              <div
                className="flex-none border-t border-gray-700"
          ...
          <div className="h-full flex flex-col overflow-hidden">
            <HD4TaskView
              ...
              onTaskSelect={task => {
                ...
                // Optional: Auto-switch to relevant tab if needed, but for now just set context
                // setActiveTab('kali');
              }}
            />
            {showCLI && (
