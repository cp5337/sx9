...
import { fetchGraph, applyGraphChanges, NodeChange } from "../services/ctas7-api-integration";
...
const GraphVisualization: React.FC = () => {
  ...
  const buildGraphData = (tasks: any[]) => {
    ...
    // Sync to GLAF Backend
    // This populates the backend with the frontend data
    syncToGlaf(nodes, links).catch(err => console.error("GLAF Sync Failed:", err));
  };

  const syncToGlaf = async (d3Nodes: GraphNode[], d3Links: GraphLink[]) => {
    try {
      // Check if backend already has data (optimization)
      const currentGraph = await fetchGraph();
      if (currentGraph.nodes.length > 0) {
        console.log(
          `GLAF Backend already has ${currentGraph.nodes.length} nodes. Skipping initial sync.`
        );
        return;
      }

      console.log("Syncing initial graph data to GLAF Backend...");

      const changes: NodeChange[] = d3Nodes.map(n => ({
        type: "add",
        item: {
          id: n.id,
          type: "custom", // Use a generic type for now
          position: { x: 0, y: 0 }, // Initial position, layouting happens in D3 or Backend
          data: {
            label: n.name,
            originalType: n.type,
            group: n.group,
            usim_hash: n.usim_hash,
            unicode: n.unicode,
          },
        },
      }));

      await applyGraphChanges(changes);
      console.log("✅ Synced nodes to GLAF Backend");
    } catch (e) {
      console.error("Failed to sync to GLAF:", e);
    }
  };
      ...
      {selectedNode && (
        <div className="absolute bottom-4 left-4 z-10 bg-gray-800 p-4 rounded-lg shadow-lg max-w-md">
          ...
          <div className="text-sm text-gray-300 space-y-1">
            ...
            <p>
              <span className="font-semibold">Group:</span> {selectedNode.group}
            </p>
            {selectedNode.usim_hash && (
              <p>
                <span className="font-semibold">USIM:</span>{" "}
                {selectedNode.usim_hash.substring(0, 16)}...
              </p>
            )}
            {selectedNode.unicode && (
              <p>
                <span className="font-semibold">Unicode:</span> {selectedNode.unicode}
              </p>
            )}
          </div>
        </div>
