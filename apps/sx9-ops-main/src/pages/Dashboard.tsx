...
import { Map, Network, Target, Server, Terminal, Database, Activity, Globe, Brain, Send, X, Settings, History, Download, RotateCcw, Grid, Wifi, WifiOff } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ view }) => {
  ...
  const [neuralMuxStatus, setNeuralMuxStatus] = useState<'connected' | 'disconnected'>('disconnected');
  ...
  useEffect(() => {
    ...
    const fetchAssets = async () => {
      try {
        // Fetch from API Gateway or Docker stats
        const response = await fetch('http://localhost:18450/api/assets');
        if (response.ok) {
  ...
  useEffect(() => {
    const initializeSynaptixCore = async () => {
      setNeuralMuxStatus(synaptixCore.getConnectionStatus() === 'connected' ? 'connected' : 'disconnected');
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case 'raptor': return <Target size={16} />;
      case 'vkali': return <Terminal size={16} />;
  ...
  const sendChatMessage = async (message: string) => {
    ...
    // Send to real AI CLI via Neural Mux
    try {
      const response = await fetch('http://localhost:50051/ai-cli/chat', {
        method: 'POST',
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className={`font-medium ${getStatusColor(selectedAsset.status)}`}>{selectedAsset.status}</p>
                  </div>
                  <div>
                ...
                <div>
                  <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Performance Metrics</h3>
                  <div className="grid grid-cols-3 gap-4">
                    <div className="bg-gray-50 dark:bg-gray-700 p-3 rounded">
                      <div className="text-2xl font-bold text-blue-600">{selectedAsset.metrics.cpu}%</div>
                      <div className="text-xs text-gray-500">CPU Usage</div>
                    </div>
                    <div className="bg-gray-50 dark:bg-gray-700 p-3 rounded">
                      <div className="text-2xl font-bold text-green-600">{selectedAsset.metrics.memory}%</div>
                      <div className="text-xs text-gray-500">Memory Usage</div>
                    </div>
                    <div className="bg-gray-50 dark:bg-gray-700 p-3 rounded">
                      <div className="text-2xl font-bold text-purple-600">{selectedAsset.metrics.network}%</div>
