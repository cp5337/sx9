...
/// CTAS Task Binary: Targeting Critical Infrastructure
...
async fn main() {
    ...
    info!("🎯 CTAS Task: Targeting Critical Infrastructure");
    info!("📍 Target: {}", target);
...
fn escalate_to_container(target: &str) {
    ...
    let _ = Command::new("docker")
        .args(&["run", "--rm", "ctas7/kali-tools:7.3.1", "uuid-007-003-001", target])
        .spawn();
}
