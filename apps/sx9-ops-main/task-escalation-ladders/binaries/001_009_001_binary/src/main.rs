...
/// CTAS Task Binary: Malware Deployment for Access
/// Task ID: uuid-001-009-001
...
async fn main() {
    ...
    info!("🎯 CTAS Task: Malware Deployment for Access");
    info!("📍 Target: {}", target);
...
fn escalate_to_container(target: &str) {
    ...
    let _ = Command::new("docker")
        .args(&["run", "--rm", "ctas7/kali-tools:7.3.1", "uuid-001-009-001", target])
        .spawn();
}
