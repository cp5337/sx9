...
/// CTAS Task Binary: Industrial Economic Disruption
...
async fn main() {
    ...
    info!("🎯 CTAS Task: Industrial Economic Disruption");
    info!("📍 Target: {}", target);
...
fn escalate_to_container(target: &str) {
    ...
    let _ = Command::new("docker")
        .args(&["run", "--rm", "ctas7/kali-tools:7.3.1", "uuid-007-003-008", target])
        .spawn();
}
