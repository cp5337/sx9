...
/// CTAS Task Binary: Vulnerability Scanning
/// Task ID: uuid-001-007-001
...
async fn main() {
    ...
    info!("🎯 CTAS Task: Vulnerability Scanning");
    info!("📍 Target: {}", target);
...
fn escalate_to_container(target: &str) {
    ...
    let _ = Command::new("docker")
        .args(&["run", "--rm", "ctas7/kali-tools:7.3.1", "uuid-001-007-001", target])
        .spawn();
}
