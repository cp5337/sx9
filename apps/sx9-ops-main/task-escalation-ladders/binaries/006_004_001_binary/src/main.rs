...
/// CTAS Task Binary: Network Trust Exploitation
/// Task ID: uuid-006-004-001
...
async fn main() {
    ...
    info!("🎯 CTAS Task: Network Trust Exploitation");
    info!("📍 Target: {}", target);
...
fn escalate_to_container(target: &str) {
    ...
    let _ = Command::new("docker")
        .args(&["run", "--rm", "ctas7/kali-tools:7.3.1", "uuid-006-004-001", target])
        .spawn();
}
