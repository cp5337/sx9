...
/// CTAS Task Binary: Cyber Exploitation for Access
/// Task ID: uuid-006-002-001
...
async fn main() {
    ...
    info!("🎯 CTAS Task: Cyber Exploitation for Access");
    info!("📍 Target: {}", target);
...
fn escalate_to_container(target: &str) {
    ...
    let _ = Command::new("docker")
        .args(&["run", "--rm", "ctas7/kali-tools:7.3.1", "uuid-006-002-001", target])
        .spawn();
}
