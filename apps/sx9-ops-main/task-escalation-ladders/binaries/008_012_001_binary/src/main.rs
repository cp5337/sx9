...
/// CTAS Task Binary: Exploiting Legal Systems
/// Task ID: uuid-008-012-001
...
async fn main() {
    ...
    info!("🎯 CTAS Task: Exploiting Legal Systems");
    info!("📍 Target: {}", target);
...
fn escalate_to_container(target: &str) {
    ...
    let _ = Command::new("docker")
        .args(&["run", "--rm", "ctas7/kali-tools:7.3.1", "uuid-008-012-001", target])
        .spawn();
}
