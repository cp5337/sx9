...
/// CTAS Task Binary: Airspace Exploitation
/// Task ID: uuid-011-013-001
...
async fn main() {
    ...
    info!("🎯 CTAS Task: Airspace Exploitation");
    info!("📍 Target: {}", target);
...
fn escalate_to_container(target: &str) {
    ...
    let _ = Command::new("docker")
        .args(&["run", "--rm", "ctas7/kali-tools:7.3.1", "uuid-011-013-001", target])
        .spawn();
}
