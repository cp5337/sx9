...
/// CTAS Task Binary: Transport Supply Chain Attacks
...
async fn main() {
    ...
    info!("🎯 CTAS Task: Transport Supply Chain Attacks");
    info!("📍 Target: {}", target);
...
fn escalate_to_container(target: &str) {
    ...
    let _ = Command::new("docker")
        .args(&["run", "--rm", "ctas7/kali-tools:7.3.1", "uuid-007-003-006", target])
        .spawn();
}
