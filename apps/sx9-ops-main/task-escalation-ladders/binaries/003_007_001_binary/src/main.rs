...
/// CTAS Task Binary: Compartmentalized Infrastructure
...
async fn main() {
    ...
    info!("🎯 CTAS Task: Compartmentalized Infrastructure");
    info!("📍 Target: {}", target);
...
fn escalate_to_container(target: &str) {
    ...
    let _ = Command::new("docker")
        .args(&["run", "--rm", "ctas7/kali-tools:7.3.1", "uuid-003-007-001", target])
        .spawn();
}
