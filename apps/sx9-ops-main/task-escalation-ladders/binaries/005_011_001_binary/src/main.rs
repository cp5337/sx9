...
/// CTAS Task Binary: Critical Infrastructure Targeting
...
async fn main() {
    ...
    info!("🎯 CTAS Task: Critical Infrastructure Targeting");
    info!("📍 Target: {}", target);
...
fn escalate_to_container(target: &str) {
    ...
    let _ = Command::new("docker")
        .args(&["run", "--rm", "ctas7/kali-tools:7.3.1", "uuid-005-011-001", target])
        .spawn();
}
