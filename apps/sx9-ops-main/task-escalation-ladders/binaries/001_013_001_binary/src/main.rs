...
/// CTAS Task Binary: Supply Chain Vulnerability ID
/// Task ID: uuid-001-013-001
...
async fn main() {
    ...
    info!("🎯 CTAS Task: Supply Chain Vulnerability ID");
    info!("📍 Target: {}", target);
...
fn escalate_to_container(target: &str) {
    ...
    let _ = Command::new("docker")
        .args(&["run", "--rm", "ctas7/kali-tools:7.3.1", "uuid-001-013-001", target])
        .spawn();
}
