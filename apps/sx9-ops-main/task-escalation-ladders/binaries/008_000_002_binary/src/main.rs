...
/// CTAS Task Binary: Multi-Jurisdiction Contingent Assembly
...
async fn main() {
    ...
    info!("🎯 CTAS Task: Multi-Jurisdiction Contingent Assembly");
    info!("📍 Target: {}", target);
...
fn escalate_to_container(target: &str) {
    ...
    let _ = Command::new("docker")
        .args(&["run", "--rm", "ctas7/kali-tools:7.3.1", "uuid-008-000-002", target])
        .spawn();
}
