...
async fn main() {
    ...
    let addr = SocketAddr::from(([0, 0, 0, 0], 18110));
    info!("🚀 Smart CDN Gateway starting on {}", addr);
...
async fn metrics() -> String {
    "# HELP cdn-gateway_requests_total Total requests\n".to_string()
}

async fn register_with_port_manager() {
    let client = reqwest::Client::new();
    let registration = PortManagerRegistration {
        service_name: "cdn-gateway".to_string(),
        port: 18110,
