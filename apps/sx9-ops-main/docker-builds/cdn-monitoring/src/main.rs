...
async fn main() {
    ...
    let addr = SocketAddr::from(([0, 0, 0, 0], 18109));
    info!("🚀 Monitoring CDN starting on {}", addr);
...
async fn health_check() -> Json<HealthResponse> {
    Json(HealthResponse {
        ...
        service: "cdn-monitoring".to_string(),
        port: 18109,
...
async fn metrics() -> String {
    "# HELP cdn-monitoring_requests_total Total requests\n".to_string()
}

async fn register_with_port_manager() {
    let client = reqwest::Client::new();
    let registration = PortManagerRegistration {
        service_name: "cdn-monitoring".to_string(),
        port: 18109,
