...
import { Network, Search, Filter, Maximize, Download, X, Wifi, WifiOff, RefreshCw } from "lucide-react"
...
import { useForgeData, useForgeStatus } from "@/lib/use-forge-data"

export default function GraphViewer() {
  // Fetch data from Forge API with mock fallback
  const { data, loading, connected, refresh } = useForgeData(true, 30000)
  const { status: forgeStatus } = useForgeStatus()

  // Use Forge data or fallback to mock
  const nodes = data?.nodes ?? mockNodes
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {connected ? `v${forgeStatus?.version || '7.3.1'}` : 'Mock Data'}
            </Badge>
          </div>
          <Badge
            variant="outline"
