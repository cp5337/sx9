import { forgeClient, forgeNodeToGraphNode, type ForgeNode, type MissionLoad } from './forge-client'
...
// ============================================================================
// SX9 Forge API Integration
// ============================================================================

export interface ForgeGraphData {
  nodes: GraphNode[]
  missionLoads: MissionLoad[]
  status: {
    connected: boolean
    nodeCount: number
    edgeCount: number
...
// Fetch graph data from Forge API with mock fallback
export async function fetchGraphData(): Promise<ForgeGraphData> {
  try {
    // Attempt Forge API connection
    const [status, forgeNodes, missionLoads] = await Promise.all([
      forgeClient.getStatus(),
      forgeClient.getNodes(),
      forgeClient.getMissionLoads(),
    ])

    // Convert Forge nodes to GraphNode format
    const nodes = forgeNodes.map(forgeNodeToGraphNode)

    return {
      nodes,
      missionLoads: missionLoads.map(ml => ({
        id: ml.id,
        name: ml.name,
        description: `${ml.hd4_phase} phase - ${ml.clearance} clearance`,
        hd4_phase: ml.hd4_phase as 'Hunt' | 'Detect' | 'Disrupt' | 'Disable' | 'Dominate',
        clearance: ml.clearance as 'Public' | 'Commercial' | 'Restricted' | 'Classified',
        primitives: [],
        primitive_bitfield: 0,
        price_credits: ml.price_credits,
        active: true,
      })),
      status: {
        connected: true,
        nodeCount: status.graph_nodes,
        edgeCount: status.graph_edges,
        version: status.version,
      },
    }
  } catch (error) {
    console.warn('Forge API unavailable, using mock data:', error)
    ...
    return {
      ...
      status: {
        ...
        edgeCount: mockNodes.reduce((acc, n) => acc + n.relationships.length, 0),
        version: 'mock',
      },
    }
  }
...
// Create tool from mission load via Forge API
export async function createToolFromMissionLoad(loadId: string) {
  return forgeClient.createToolFromLoad(loadId)
}

// Execute tool via Ring Bus L2
export async function executeTool(toolId: string) {
  return forgeClient.executeTool(toolId)
}

// Get Forge service health
export async function getForgeHealth() {
  try {
    return await forgeClient.getHealth()
  } catch {
    return { status: 'offline', service: 'sx9-forge', version: 'unknown' }
  }
}

// Re-export forge client for direct access
export { forgeClient }
