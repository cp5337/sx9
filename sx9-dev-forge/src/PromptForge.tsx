...
const PROMPT_TYPES = {
  ...
  THREAT_EMULATION: {
    ...
    hardConstraints: [
      ...
      "Log ALL actions",
      "HALT on unintended impact",
    ],
    softConstraints: [
      "Map all actions to MITRE ATT&CK",
  ...
  CODE_GENERATION: {
    ...
    hardConstraints: [
      ...
      "DO NOT modify outside designated module",
  ...
  RFC_ALIGNMENT: {
    ...
    hardConstraints: [
      ...
      "DO NOT modify RFC content except alignments",
      "NO per-RFC directories",
    ],
    softConstraints: [
      "Use shared LaTeX system",
  ...
  FACTORY_PIPELINE: {
    name: "Kali Factor Pipeline",
    icon: "🏭",
    persona: "FORGE",
    phase: "IMPLEMENT",
    priority: "P0",
    hardConstraints: [
      "ZERO-INTERVENTION: Agent must fix own errors",
      "BIRTH-CERTIFICATE: Must generate crate_interview.json",
      "REGISTRY-COMPLIANT: Must pass sx9-lightning-qa",
    ],
    softConstraints: [
      "Use approved crates only",
      "Log all self-correction attempts",
    ],
    linearLabels: ["type:KALI-TOOL", "phase:IMPLEMENT", "bot:factory"],
    contextSources: { memory: true, linear: true, drive: true, web: true },
  },
};

// ═══════════════════════════════════════════════════════════════════════════════
// PATTERN REGISTRY (The "Element")
// ═══════════════════════════════════════════════════════════════════════════════

const PATTERNS = {
  reactor: {
    name: "Reactor",
    desc: "High-concurrency event loop (Rust)",
    type: "rust_crate",
  },
  facade: {
    name: "Facade",
    desc: "Unified interface for subsystems (Python)",
    type: "python_script",
  },
  strategy: {
    name: "Strategy",
    desc: "Interchangeable algorithms (Python/Rust)",
    type: "tofu_module",
  },
  observer: {
    name: "Observer",
    desc: "Event listener/Dashboard (React)",
    type: "react_component",
  },
  adapter: {
    name: "Adapter",
    desc: "Interface translation layer",
    type: "rust_crate",
  },
  builder: {
    name: "Builder",
    desc: "Step-by-step object construction",
    type: "rust_crate",
  },
};
...
export default function PromptForge() {
  ...
  const [form, setForm] = useState<FormState>({
    ...
    // Pattern Element
    pattern: "reactor",
    patternType: "rust_crate",
    cl_action: "Route",
    cl_subject: "Network Traffic",
    cl_quality: "Real-time",
    foundations: { "async-nats": true, murmur3: true, tokio: true },
    jobDescription: "enforce zero-trust checks on incoming packet streams",

    // Factory Defaults
    toolName: "kali-tool",
    category: "Recon",
    capabilityList: "nmap, netcat",
    nvnn: "Message-Route-Trust-Verify",
    foundation: "async-nats, murmur3",
  });
  ...
  const set = (k: string, v: string | boolean | Record<string, boolean>) =>
    setForm((f) => ({ ...f, [k]: v }));
  ...
  const generate = useCallback(() => {
    ...
    // Assemble Foundation String
    const selectedFoundations = Object.entries(form.foundations)
      .filter(([_, v]) => v)
      .map(([k]) => k)
      .join(", ");

    // CONSTUCT FORMAL OUTPUT from Common Language
    // User sees: "I want to Route Network Traffic"
    // System generates: "creating a Reactor to perform Message-Route-Verify..."

    let objectiveSection = "";
    let mandatoryOutputs = "";

    if (promptType === "FACTORY_PIPELINE") {
      objectiveSection = `OBJECTIVE:
  "You are the SX9 FACTORY AGENT. Your mission is to manufacture a 'Kali Tool Crate' 
   from the following specifications with ZERO human intervention."

INPUT_VECTOR:
  ToolName: "${form.toolName}"
  Category: "${form.category}"
  Capabilities: "${form.capabilityList}"
  ConstraintLevel: "RFC-9101 STRICT"`;

      mandatoryOutputs = `MANDATORY_OUTPUTS:
  1. CODEBASE:
     - Language: Python (via sx9-conda) OR Rust.
     - Structure: /crates/${form.toolName} OR /tools/${form.toolName}
     - Dockerfile: Multi-stage, distroless base.
  
  2. BIRTH_CERTIFICATE (RFC-9025):
     - File: crate_interview.json
     - Content: MUST include H1/H2 hashes, full Capability Map, and Dependency Graph.
     - Validation: This file is the "Ticket to Ride" for the Registry.

  3. VALIDATION_PROOF:
     - File: lightning-qa.json
     - Status: MUST be Grade A (>85).
`;
    } else {
      objectiveSection = `objective: |
  ${form.objective || `...creating a ${form.pattern} ${form.patternType} to ${form.cl_action} ${form.cl_subject} in a ${form.cl_quality} manner, using ${selectedFoundations} components; its job is to ${form.jobDescription}.`}`;
    }

    const yaml = `# SX9-PROMPT v3.0 (Factory Edition)
...
analysis_placeholder: |
  ...
  - Architecture: ${PATTERNS[form.pattern as keyof typeof PATTERNS]?.desc}
  - Core Function: ${form.cl_action} (${form.cl_subject})
  - Key Attribute: ${form.cl_quality}
  - Dependencies: ${selectedFoundations}
  - Complexity: Estimated Medium (<1k LOC) based on ${form.patternType}
  - Risks: Check for thread-safety in ${form.patternType} and compliance in foundation crates.
        ...
        <div className="flex items-center justify-between mb-4 pb-3 border-b border-zinc-700">
          <div>
            ...
            <p className="text-xs text-zinc-500">
              Prompt Engineering • Linear • IDE Bootstrap
            </p>
          </div>
          <div className="flex gap-2">
            ...
            <button
              onClick={download}
              disabled={!output}
              className="px-4 py-2 bg-zinc-700 hover:bg-zinc-600 disabled:opacity-50 rounded text-sm"
            >
              DOWNLOAD
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          ...
          <div className="lg:col-span-3 space-y-3">
            ...
            <div className="flex gap-1 border-b border-zinc-700 pb-2">
              {["config", "pattern", "tools", "linear", "mission"].map(
                (tab) => (
                  <button
                    key={tab}
                ...
                <div className="bg-zinc-800/50 rounded p-3 border border-zinc-700">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      >
                        {["INTERNAL", "SENSITIVE", "PUBLIC"].map((c) => (
                ...
                <div className="bg-zinc-800/50 rounded p-3 border border-zinc-700">
                  <div className="grid grid-cols-4 gap-3">
                    <div>
                      ...
                      >
                        {["AUTONOMOUS", "SUPERVISED", "STEP-CONFIRM"].map(
                          (m) => (
                    ...
                    <div>
                      ...
                      >
                        {["HALT", "ROLLBACK", "ESCALATE", "LOG-CONTINUE"].map(
                          (f) => (
            ...
            {/* Pattern Tab (The Element) */}
            {activeTab === "pattern" && (
              <div className="space-y-3">
                <div className="bg-emerald-900/20 rounded p-3 border border-emerald-700/50">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-emerald-400">⚡</span>
                    <label className={labelClass + " !mb-0 text-emerald-100"}>
                      PATTERN RESOLUTION PROTOCOL (N-V-N-N)
                    </label>
                  </div>
                  <p className="text-xs text-zinc-400 mb-4">
                    Strictly enforcing the{" "}
                    <i>"...creating a [PATTERN] [TYPE]..."</i> structure.
                  </p>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          const p =
                            PATTERNS[e.target.value as keyof typeof PATTERNS];
                          setForm((f) => ({
                            ...f,
                            pattern: e.target.value,
                            patternType: p ? p.type : f.patternType,
                          }));
                        }}
                      >
                  ...
                  <div className="space-y-3">
                    <div>
                      <label className={labelClass}>
                        Logic Vector (N-V-N-N)
                      </label>
                      <input
                        className={inputClass}
                        value={form.nvnn}
                        onChange={(e) => set("nvnn", e.target.value)}
                        placeholder="Message-Route-Trust-Verify"
                      />
                      <p className="text-[10px] text-zinc-500 mt-1">
                        Foundational logic flow (Noun-Verb-Noun-Noun)
                      </p>
                    </div>
                    <div>
                      <label className={labelClass}>
                      ...
                      <input
                        ...
                        onChange={(e) => set("foundation", e.target.value)}
                        placeholder="async-nats, murmur3"
                      />
                    </div>
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder="enforce zero-trust checks..."
                      />
                    </div>
                  </div>
                </div>

                <div className="p-3 bg-zinc-900 rounded border border-zinc-700 font-mono text-xs text-zinc-400">
                  <span className="text-zinc-500">PREVIEW:</span>
                  <br />
                  "...creating a{" "}
                  <span className="text-emerald-400">{form.pattern}</span>{" "}
                  <span className="text-cyan-400">{form.patternType}</span> to
                  perform this{" "}
                  <span className="text-amber-400">{form.nvnn}</span> task,
                  using{" "}
                  <span className="text-purple-400">{form.foundation}</span>{" "}
                  components; its job is to{" "}
                  <span className="text-zinc-100">{form.jobDescription}</span>."
                </div>
              </div>
            )}
            ...
            {activeTab === "tools" && (
              <div className="space-y-3">
                ...
                <div className="bg-zinc-800/50 rounded p-3 border border-zinc-700">
                  ...
                  <div className="grid grid-cols-4 gap-2 mt-2">
                    {[
                      ...
                      <label
                        key={k}
                        className={`${checkClass} p-2 rounded border ${form[k] ? "border-cyan-700 bg-cyan-900/20" : "border-zinc-700"}`}
                      >
                        <input
                          type="checkbox"
                          checked={form[k] as boolean}
                ...
                <div className="bg-zinc-800/50 rounded p-3 border border-zinc-700">
                  ...
                  <input
                    className={inputClass}
                    value={form.forbidPaths}
                    onChange={(e) => set("forbidPaths", e.target.value)}
                  />
                </div>
              </div>
            )}
              ...
              <div className="space-y-3">
                <div className="bg-zinc-800/50 rounded p-3 border border-zinc-700">
                  <label className={`${checkClass} mb-3`}>
                    <input
                      ...
                      onChange={(e) =>
                        set("createLinearIssue", e.target.checked)
                      }
                      className="w-4 h-4"
                    />
                    <span className="text-sm font-medium">
                      Create Linear Issue for this mission
                    </span>
                  </label>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => set("linearProject", e.target.value)}
                    ...
                    <div className="col-span-2">
                      ...
                      <input
                        className={inputClass}
                        value={form.linearLabels}
                        onChange={(e) => set("linearLabels", e.target.value)}
                        placeholder="type:BUILD, phase:IMPLEMENT"
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-zinc-800/50 rounded p-3 border border-zinc-700">
                  ...
                  <div className="text-xs text-zinc-400 space-y-1 mt-2">
                    <p>
                      ✓ <strong>On Start:</strong> Create issue with mission
                      details, add labels
                    </p>
                    <p>
                      ✓ <strong>On Checkpoint:</strong> Add comment with
                      progress update
                    </p>
                    <p>
                      ✓ <strong>On Complete:</strong> Update to Done, link
                      artifacts
                    </p>
                    <p>
                      ✓ <strong>On Fail:</strong> Update status, document
                      failure reason
                    </p>
                  </div>
                </div>
