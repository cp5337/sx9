...
export class HashingEngineConnector {
  ...
  async initialize(): Promise<boolean> {
    ...
    try {
      ...
      if (this.isConnected) {
        ...
        console.log(
          `⚡ Supported Algorithms: ${this.status?.supportedAlgorithms.join(
            ", "
          )}`
        );
        return true;
  ...
  async hashData(request: HashRequest): Promise<HashResponse> {
    ...
    console.log(
      `🔐 Hashing data with ${request.algorithm || "murmur3"} algorithm`
    );
    ...
    try {
      const response = await fetch(`${this.baseUrl}/hash`, {
        ...
        body: JSON.stringify({
          data: request.data,
          algorithm: request.algorithm || "murmur3",
          format: request.format || "base96",
  ...
  async batchHash(
    ...
    console.log(
      `🔐 Processing batch hash: ${batchId} (${requests.length} items, ${priority} priority)`
    );
    ...
    try {
      ...
      console.log(
        `✅ Batch completed: ${result.completedCount}/${requests.length} successful`
      );
  ...
  async hashForThreatIntelligence(
    indicators: string[]
  ): Promise<Record<string, string>> {
    console.log(
      `🛡️ Hashing ${indicators.length} threat intelligence indicators`
    );
    ...
    console.log(
      `✅ Hashed ${Object.keys(hashedIndicators).length} threat indicators`
    );
    return hashedIndicators;
  }

  async hashForDocumentManager(
    documents: Array<{ id: string; content: string }>
  ): Promise<Array<{ id: string; hash: string; compressed: boolean }>> {
    ...
    const hashedDocs = batchResult.results
      .map((result, index) => ({
  ...
  async hashForLegionTasks(
    tasks: Array<{ id: string; script: string; world: string }>
  ): Promise<Array<{ taskId: string; scriptHash: string; worldHash: string }>> {
  ...
  async getPerformanceMetrics(): Promise<{
    ...
    return {
      ...
      compressionEfficiency:
        (this.status.performance.totalCompressions /
          Math.max(this.status.performance.totalRequests, 1)) *
        100,
  ...
  getQueueStatus(): {
    ...
    const queuedItems = Array.from(this.requestQueue.values()).reduce(
