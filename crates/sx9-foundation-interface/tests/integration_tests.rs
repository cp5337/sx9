...
async fn test_interface_service_integration() {
    ...
    // Test URL parsing
    let url = service
        .parse_url("https://example.com/test?param=value")
        .unwrap();
...
async fn test_http_operations() {
    ...
    let response = service
        .http_post_json("https://httpbin.org/post", &data)
        .await
        .unwrap();
...
async fn test_cli_parsing() {
    ...
    // SAFETY: This is a test, race conditions on env vars are acceptable risks here
    unsafe {
...
async fn test_websocket_router_creation() {
    ...
    let _router = service.create_websocket_router();
...
async fn test_custom_configuration() {
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("HTTP client not enabled"));
