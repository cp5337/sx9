...
impl ComponentCDN {
    ...
    pub async fn initialize_ctas_components(&mut self) {
        let ctas_components = vec![
            ...
            ComponentEntry {
                ...
                component_type: ComponentType::React,
                visualization_methods: vec![VisualizationMethod::Grid, VisualizationMethod::Table],
    ...
    pub fn get_components_by_category(&self, category: &ComponentCategory) -> Vec<&ComponentEntry> {
        self.components
            .values()
            .filter(|component| {
                std::mem::discriminant(&component.category) == std::mem::discriminant(category)
            })
            .collect()
    }

    /// Get components by visualization method
    pub fn get_components_by_visualization_method(
        &self,
    ...
    pub fn get_component_stats(&self) -> Value {
        ...
        for component in self.components.values() {
            *category_counts
                .entry(format!("{:?}", component.category))
                .or_insert(0) += 1;
            *type_counts
                .entry(format!("{:?}", component.component_type))
                .or_insert(0) += 1;
...
pub fn create_component_cdn_routes() -> Router {
    Router::new()
        ...
        .route(
            ...
            get(get_components_by_category),
        )
        .route(
            "/components/visualization/{method}",
            get(get_components_by_visualization),
        )
