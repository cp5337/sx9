...
impl PowerShellBeaconDissector {
    ...
    pub async fn analyze_script(&self, script: &[u8]) -> Result<PowerShellAnalysisResult> {
        ...
        let beacon_config = self.extract_beacon_configuration(&script_content, &deobfuscation_results)?;
        ...
        let mitre_mappings = self.generate_mitre_mappings(&malicious_functions, &evasion_techniques)?;
        ...
        let result = PowerShellAnalysisResult {
            ...
            analysis_metadata: AnalysisMetadata {
                ...
                engines_used: self.deobfuscation_engines.iter()
                    .map(|e| e.name.clone())
                    .collect(),
        ...
        info!("✅ PowerShell analysis completed: {} ({}ms)",
              analysis_id, analysis_duration.as_millis());
    ...
    async fn resolve_string_concatenation(&self, script: &str) -> Result<DeobfuscationResult> {
        ...
        Ok(DeobfuscationResult {
            ...
            success,
            deobfuscated_code: if success { Some(resolved_strings.join("\n")) } else { None },
    ...
    fn detect_c2_framework(
        ...
        for result in deob_results {
            if let Some(deob_code) = &result.deobfuscated_code {
                self.check_framework_patterns(deob_code, &mut framework_scores, &mut all_indicators);
            }
        }
        ...
        for (sig_name, signature) in &self.malware_signatures {
            ...
            if match_count > 0 {
                ...
                if confidence >= signature.confidence_threshold {
                    *framework_scores.entry(signature.framework.clone()).or_insert(0.0) += confidence;
                }
            }
        }

        // Determine best match
        if let Some((framework, &confidence)) = framework_scores.iter()
        ...
        for (_, pattern) in &self.c2_patterns {
            if let Ok(regex) = Regex::new(&pattern.pattern) {
                if let Some(m) = regex.find(script) {
                    *scores.entry(pattern.framework.clone()).or_insert(0.0) += pattern.confidence_weight;
    ...
    fn analyze_malicious_functions(&self, script: &str) -> Result<Vec<MaliciousFunction>> {
        ...
        let network_patterns = [
            (r"(?i)New-Object.*Net\.WebClient", "WebClient Creation", MaliciousFunctionType::NetworkConnection),
            (r"(?i)Invoke-WebRequest", "Web Request", MaliciousFunctionType::NetworkConnection),
            (r"(?i)wget", "File Download", MaliciousFunctionType::FileDownload),
            (r"(?i)curl", "File Download", MaliciousFunctionType::FileDownload),
        ];
        ...
        let injection_patterns = [
            (r"(?i)VirtualAlloc", "Memory Allocation", MaliciousFunctionType::ProcessInjection),
            (r"(?i)WriteProcessMemory", "Process Memory Write", MaliciousFunctionType::ProcessInjection),
            (r"(?i)CreateRemoteThread", "Remote Thread Creation", MaliciousFunctionType::ProcessInjection),
        ];
    ...
    fn analyze_file_operations(&self, script: &str) -> Result<Vec<FileOperation>> {
        ...
        let file_patterns = [
            (r"(?i)Get-Content\s+([^\s]+)", "File Read", FileOperationType::Read),
            (r"(?i)Set-Content\s+([^\s]+)", "File Write", FileOperationType::Write),
            (r"(?i)Remove-Item\s+([^\s]+)", "File Delete", FileOperationType::Delete),
            (r"(?i)Copy-Item\s+([^\s]+)", "File Copy", FileOperationType::Copy),
        ];
    ...
    fn analyze_registry_operations(&self, script: &str) -> Result<Vec<RegistryOperation>> {
        ...
        let reg_patterns = [
            (r"(?i)Get-ItemProperty\s+([^\s]+)", "Registry Read", RegistryOperationType::GetValue),
            (r"(?i)Set-ItemProperty\s+([^\s]+)", "Registry Write", RegistryOperationType::SetValue),
            (r"(?i)New-ItemProperty\s+([^\s]+)", "Registry Create", RegistryOperationType::SetValue),
            (r"(?i)Remove-ItemProperty\s+([^\s]+)", "Registry Delete", RegistryOperationType::DeleteValue),
        ];
    ...
    fn analyze_process_operations(&self, script: &str) -> Result<Vec<ProcessOperation>> {
        ...
        let proc_patterns = [
            (r"(?i)Start-Process\s+([^\s]+)", "Process Start", ProcessOperationType::Create),
            (r"(?i)Stop-Process\s+([^\s]+)", "Process Stop", ProcessOperationType::Kill),
            (r"(?i)Get-Process", "Process List", ProcessOperationType::ListProcesses),
        ];
    ...
    fn generate_mitre_mappings(
        ...
        for func in malicious_functions {
            for technique in &func.mitre_techniques {
                let mapping = mappings.entry(technique.clone()).or_insert_with(|| {
                    MitreMapping {
        ...
        for evasion in evasion_techniques {
            let mapping = mappings.entry(evasion.mitre_technique.clone()).or_insert_with(|| {
                MitreMapping {
    ...
    fn perform_risk_assessment(
        ...
        if let Some(detection) = c2_detection {
            ...
            risk_factors.push(RiskFactor {
                ...
                description: format!("Detected {} framework", format!("{:?}", detection.framework)),
            });
        ...
        if !malicious_functions.is_empty() {
            let weight = 0.3;
            let critical_count = malicious_functions.iter()
                .filter(|f| matches!(f.risk_level, RiskLevel::Critical))
                .count() as f64;
            let high_count = malicious_functions.iter()
        ...
        if !evasion_techniques.is_empty() {
            let weight = 0.2;
            let avg_confidence = evasion_techniques.iter()
                .map(|e| e.confidence)
                .sum::<f64>() / evasion_techniques.len() as f64;
    ...
    fn generate_recommendations(&self, risk_level: &RiskLevel, factors: &[RiskFactor]) -> Vec<String> {
        ...
        match risk_level {
            RiskLevel::Critical => {
                recommendations.push("IMMEDIATE ACTION REQUIRED: Isolate affected systems".to_string());
                recommendations.push("Block all network communications to identified C2 servers".to_string());
            ...
            RiskLevel::High => {
                ...
                recommendations.push("Monitor network traffic for beacon communications".to_string());
            ...
            RiskLevel::Medium => {
                ...
                recommendations.push("Implement additional monitoring for suspicious activities".to_string());
            }
            _ => {
        ...
        for factor in factors {
            match factor.factor.as_str() {
                ...
                "Evasion Techniques" => {
                    recommendations.push("Update security tools to detect evasion techniques".to_string());
                }
                _ => {}
            }
        }
    ...
    fn calculate_string_entropy(&self, script: &str) -> f64 {
        ...
        let strings: Vec<&str> = string_regex.captures_iter(script)
    ...
    fn calculate_variable_entropy(&self, script: &str) -> f64 {
        ...
        let variables: Vec<&str> = var_regex.captures_iter(script)
            .filter_map(|cap| cap.get(1))
            .map(|m| m.as_str())
            .collect();
    ...
    fn calculate_function_entropy(&self, script: &str) -> f64 {
        ...
        let functions: Vec<&str> = func_regex.captures_iter(script)
            .filter_map(|cap| cap.get(1))
            .map(|m| m.as_str())
            .collect();
        ...
        for capture in string_regex.captures_iter(script) {
            if let Some(string_match) = capture.get(1) {
                ...
                if string_value.len() > 20 &&
                   Regex::new(r"^[A-Za-z0-9+/]+=*$").unwrap().is_match(string_value) {
                    base64_candidates.push(string_value.to_string());
                }

                // Check for hex
                if Regex::new(r"^[0-9a-fA-F]+$").unwrap().is_match(string_value) &&
                   string_value.len() % 2 == 0 && string_value.len() > 10 {
    ...
    fn classify_suspicious_string(&self, string_value: &str) -> SuspiciousStringType {
        ...
        } else if Regex::new(r"(?i)(powershell|cmd|rundll32)").unwrap().is_match(string_value) {
            SuspiciousStringType::SystemCommand
        } else if Regex::new(r"(?i)(HKEY_|HKLM|HKCU)").unwrap().is_match(string_value) {
            SuspiciousStringType::RegistryPath
        } else if Regex::new(r"(?i)[A-Za-z0-9+/]{20,}={0,2}").unwrap().is_match(string_value) {
