...
use crate::traffic_types::{SuspiciousActivity, RequestData, AnalysisResult, ResponseAction};
...
impl TrafficAnalysis {
    ...
    fn matches_attack_pattern(&self, path: &str, payload: &str) -> bool {
        let attack_patterns = [
            ...
            "<script>", "javascript:", "eval(", "cmd=",
        ];
        let combined = format!("{} {}", path, payload).to_lowercase();
        attack_patterns.iter().any(|&pattern| combined.contains(&pattern.to_lowercase()))
    }
