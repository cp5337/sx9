...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(packages) = metadata["packages"].as_array() {
        println!(
            "📊 Analyzing {} crates for foundation migration...",
            packages.len()
        );
...
fn display_migration_targets(targets: &[CrateMigrationTarget]) {
    ...
    for &count in counts {
        ...
        for target in group {
            ...
            println!(
                "  {}: {} foundation deps ({:.1}%), {:.1}MB savings",
                target.name,
                target.foundation_deps.len(),
                foundation_percentage,
                target.memory_savings_mb
            );
        }
    }
}
...
fn display_effort_breakdown(targets: &[CrateMigrationTarget]) {
    ...
    for target in targets {
        effort_groups
            .entry(target.migration_effort.clone())
            .and_modify(|group| group.push(target))
            .or_insert_with(|| vec![target]);
    }
    ...
    for effort in effort_order {
        if let Some(group) = effort_groups.get(&effort) {
            ...
            let avg_deps: f64 =
                group.iter().map(|t| t.dependency_count as f64).sum::<f64>() / group.len() as f64;
            ...
            sorted_group.sort_by(|a, b| {
                b.memory_savings_mb
                    .partial_cmp(&a.memory_savings_mb)
                    .unwrap()
            });
            ...
            for target in sorted_group.iter().take(5) {
                println!(
                    "    {}: {:.1}MB savings",
                    target.name, target.memory_savings_mb
                );
            }
        }
    }
...
fn display_migration_plan(targets: &[CrateMigrationTarget]) {
    ...
    let easy_targets: Vec<_> = targets.iter().filter(|t| t.dependency_count <= 4).collect();

    println!(
        "\n🎯 Phase 1: Easy Targets (≤4 deps) - {} crates",
        easy_targets.len()
    );
    ...
    for target in &easy_targets {
        println!(
            "  {}: {} deps → 1 foundation dep, {:.1}MB savings",
    ...
    println!(
        "  Average savings per crate: {:.1}MB",
        total_savings / easy_targets.len() as f64
    );
