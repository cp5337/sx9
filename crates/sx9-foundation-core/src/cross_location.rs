...
// sign_hmac_sha512 removed - Ground Truth: Murmur3 ONLY
...
impl CrossValidationEngine {
    ...
    /// Signs the verdict data using Murmur3 Trivariate Hash (Ground Truth).
    fn sign_verdict(&self, result: &ValidationResult) -> Result<String, CrossValidationError> {
        ...
        use base64::prelude::*;
        let content_b64 = BASE64_STANDARD.encode(&serialized);
        let key_b64 = BASE64_STANDARD.encode(self.signing_key.expose());
        
        let sig = engine.generate_trivariate_hash(&content_b64, &key_b64, "VerdictSignature");
        Ok(sig)
    }
}
