...
use crate::types::*;
use crate::cyber_operations::{CyberOperations, ActiveOperation};
...
impl GatewayCDN {
    ...
    pub async fn register_service(&self, service: ECSService) -> Result<(), CDNError> {
        ...
        // Allocate port
        self.port_manager.allocate_port(service.port, &service.name, service.service_type.clone()).await?;
    ...
    pub async fn generate_cyber_ops_config(&self, service_name: &str) -> Result<String, CDNError> {
        ...
        self.nginx_config.generate_cyber_ops_config(service_name, service.port).await
    }
    ...
    pub async fn get_gateway_status(&self) -> GatewayStatus {
        ...
        GatewayStatus {
            ...
            healthy_services: services.values().filter(|s| matches!(s.status, ServiceStatus::Healthy)).count(),
...
impl PortManager {
    ...
    pub async fn allocate_port(&self, port: u16, service_name: &str, service_type: ServiceType) -> Result<(), CDNError> {
