...
impl CTASCDN {
    ...
    pub async fn route_request(&self, request: CDNRequest) -> Result<CDNResponse, CDNError> {
        ...
        // Cache the response
        self.cache_manager.cache_response(&request, &origin_response).await?;
        
        // Update analytics
        self.analytics.record_request(&request, &origin_response).await;
    ...
    pub async fn get_system_health(&self) -> SystemHealth {
        ...
        SystemHealth {
            ...
            overall_health: if healthy_edges > 0 && healthy_origins > 0 { "Healthy" } else { "Degraded" }.to_string(),
