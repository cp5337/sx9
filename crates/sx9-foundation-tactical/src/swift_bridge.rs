...
pub enum ConstraintType {
    ...
    OrderOfBattle {
        friendly_forces: Vec<String>,
        threat_forces: Vec<String>,
    },
}
...
impl SwiftBridge {
    ...
    pub async fn validate_tool_combination(
            ...
            for constraint in constraints {
                match &constraint.constraint_type {
                    ...
                    ConstraintType::Policy {
                        ...
                        if *restriction_level > 7
                            && matches!(tool.tool_category, ToolCategory::Exploitation)
                        {
                            tool_attraction += tool.attraction_physics.constraint_repulsion as f64;
                            constraint_violations.push(format!(
        ...
        TacticalResult::success(validation_result, start.elapsed().as_millis() as f64)
    }

    /// Execute tactical playlist
    pub async fn execute_playlist(
        &self,
        playlist: &TacticalPlaylist,
    ) -> TacticalResult<serde_json::Value> {
        let start = std::time::Instant::now();

        // Generate execution hashes for the playlist
        let execution_hashes: Vec<String> = playlist
            .tools
            .iter()
