...
impl VoiceLogger {
    ...
    pub fn new(config: VoiceLoggerConfig) -> Self {
        ...
        if config.enable_thalamic_filter {
            info!("Thalamic filter enabled - attention threshold: {:.2}", 
                config.thalamic_filter.as_ref().map(|f| f.attention_threshold).unwrap_or(0.3));
        }
        
        Self {
            config,
            buffer: Arc::new(RwLock::new(Vec::with_capacity(1000))),
            total_logged: Arc::new(RwLock::new(0)),
    ...
    pub async fn log(&self, mut interaction: VoiceInteraction) -> Result<()> {
        ...
        if self.config.enable_thalamic_filter {
            ...
            match filter_result {
                ThalamicFilterResult::Filtered => {
                    debug!("Voice interaction filtered by thalamic filter: {}", interaction.interaction_id);
                    return Ok(());
                }
                ThalamicFilterResult::RateLimited => {
                    debug!("Voice interaction rate limited: {}", interaction.interaction_id);
                    return Ok(());
                }
                ThalamicFilterResult::Bypassed => {
                    debug!("Voice interaction bypassed filter (emergency): {}", interaction.interaction_id);
                }
        ...
        // Add to buffer (non-blocking)
        {
            ...
            *stats.interactions_by_speaker.entry(speaker_key).or_insert(0) += 1;
        ...
        debug!("Voice interaction logged: {} ({}μs)", interaction.interaction_id, elapsed.as_micros());
    ...
    async fn flush_buffer(&self) -> Result<()> {
        ...
        info!("Flushing {} voice interactions to storage", interactions.len());
    ...
    pub async fn query(&self, filter: VoiceQueryFilter) -> Result<Vec<VoiceInteraction>> {
        ...
        let mut results: Vec<VoiceInteraction> = buffer.iter()
            .filter(|interaction| {
                // Filter by speaker
    ...
    async fn apply_thalamic_filter(&self, interaction: &VoiceInteraction) -> Result<ThalamicFilterResult> {
        let filter_config = self.config.thalamic_filter.as_ref()
        ...
        if filter_config.speaker_filtering && !filter_config.allowed_speakers.is_empty() {
            if !filter_config.allowed_speakers.contains(&interaction.speaker) {
                state.filtered_count += 1;
        ...
        if filter_config.mode_filtering && !filter_config.allowed_modes.is_empty() {
            if !filter_config.allowed_modes.contains(&interaction.execution_mode) {
                state.filtered_count += 1;
        ...
        if filter_config.adaptive_filtering {
            ...
            state.adaptive_threshold = filter_config.attention_threshold * (1.0 + state.cognitive_load);
    ...
    fn calculate_attention_score(&self, interaction: &VoiceInteraction, state: &ThalamicFilterState) -> f32 {
        let mut score = 0.0;
...
mod tests {
    ...
    async fn test_log_update() {
        ...
        logger.log_update(VoiceSpeaker::Zoe, "Test message").await.unwrap();
    ...
    async fn test_query_by_speaker() {
        ...
        logger.log_update(VoiceSpeaker::Zoe, "Message 1").await.unwrap();
        logger.log_update(VoiceSpeaker::Natasha, "Message 2").await.unwrap();
        logger.log_update(VoiceSpeaker::Zoe, "Message 3").await.unwrap();
