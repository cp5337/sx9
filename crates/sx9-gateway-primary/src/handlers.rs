...
use crate::protocol::*;
use crate::state::{SharedState, ports};
...
pub async fn handle_message(
    ...
    match msg {
        ...
        WsMessage::Query { db, query, params } => {
            handle_query(db, query, params, state).await
        }
        ...
        WsMessage::Unsubscribe { db, table } => {
            handle_unsubscribe(db, table, state).await
        }
        ...
        WsMessage::GetGraph { filter } => {
            handle_get_graph(filter, state).await
        }
        
        WsMessage::GetFusionNodes { threshold } => {
            handle_get_fusion_nodes(threshold, state).await
        }
        
        WsMessage::ExpandNode { node_id, depth } => {
            handle_expand_node(node_id, depth, state).await
        }
        
        WsMessage::RunCorrelation { source_ids } => {
            handle_run_correlation(source_ids, state).await
        }
        ...
        WsMessage::GetWorkflows => {
            handle_get_workflows(state).await
        }
        
        WsMessage::GetWorkflow { id } => {
            handle_get_workflow(id, state).await
        }
        
        WsMessage::StartWorkflow { id, input } => {
            handle_start_workflow(id, input, state).await
        }
        
        WsMessage::StopWorkflow { id } => {
            handle_stop_workflow(id, state).await
        }
        
        WsMessage::GetPlasmaState => {
            handle_get_plasma_state(state).await
        }
        
        WsMessage::SubscribePlasma => {
            handle_subscribe_plasma(state).await
        }
        ...
        WsMessage::GetConnections => {
            handle_get_connections(state).await
        }
        
        WsMessage::TestConnection { db } => {
            handle_test_connection(db, state).await
        }
    ...
    if let Some(surreal) = surreal_guard.as_ref() {
        match surreal.query(&query).await {
            Ok(mut response) => {
                ...
                let nodes: Vec<GraphNode> = entities.iter().filter_map(|e| {
                    Some(GraphNode {
                        id: e.get("id")?.as_str()?.to_string(),
                        label: e.get("name").and_then(|n| n.as_str()).unwrap_or("").to_string(),
                        node_type: e.get("type").and_then(|t| t.as_str()).unwrap_or("default").to_string(),
                        shape: if e.get("fusion_score").is_some() { "nonagon" } else { "circle" }.to_string(),
                        ...
                        size: 1.0,
                        trivariate_hash: e.get("trivariate_hash").and_then(|h| h.as_str()).map(String::from),
...
async fn handle_get_fusion_nodes(
    threshold: f32,
    state: SharedState,
) -> Result<WsResponse> {
    let query = format!(
    ...
    if let Some(surreal) = surreal_guard.as_ref() {
        match surreal.query(&query).await {
            Ok(mut response) => {
                ...
                let nodes: Vec<FusionNode> = raw.iter().filter_map(|n| {
                    Some(FusionNode {
                        ...
                        fusion_method: n.get("fusion_method").and_then(|m| m.as_str()).unwrap_or("hash").to_string(),
                        sources: vec![], // TODO: Parse sources
                        created_at: n.get("created_at").and_then(|t| t.as_u64()).unwrap_or(0),
                        last_correlated: n.get("last_correlated").and_then(|t| t.as_u64()).unwrap_or(0),
                    })
                }).collect();
...
async fn handle_get_plasma_state(
    state: SharedState,
) -> Result<WsResponse> {
...
async fn handle_subscribe_plasma(
    _state: SharedState,
) -> Result<WsResponse> {
