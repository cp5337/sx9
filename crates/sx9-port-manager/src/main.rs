...
║   ╚══════╝╚═╝  ╚═╝ ╚════╝     ╚═╝      ╚═════╝ ╚═╝  ╚═╝   ╚═╝   
║                                                               ║
║               M A N A G E R   v0.2.0                          ║
║                                                               ║
╚═══════════════════════════════════════════════════════════════╝
"#
    );

    info!("🚀 Starting SX9 Port Manager");
    info!("📡 Port Range: 18100-18199 (Primary) | 28100-28199 (Mirror)");
    ...
    let app = Router::new()
        ...
        // Smart registration endpoints
        .route("/register", post(register_service))
        .route("/discover", get(discover_services))
        .route("/blocks", get(get_blocks))
        .with_state(port_manager);

    // Start server on real CTAS-7 port
    let listener = tokio::net::TcpListener::bind("0.0.0.0:18103")
        .await
        .unwrap();
