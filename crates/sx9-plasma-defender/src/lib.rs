...
pub use ann_daemon::{AnnDaemon, AnnObservation, AnnConfig};
...
impl PlasmaDefender {
    pub async fn new(config: DefenderConfig) -> anyhow::Result<Self> {
        ...
        let sdt_config = config.sdt_config.as_ref()
    ...
    pub async fn start(&self) -> anyhow::Result<()> {
        ...
        tokio::spawn(async move {
            if let Err(e) = crate::tool_handler::subscribe_tool_results_internal(
                plasma_bus,
                ann_daemon_clone,
            ).await {
    ...
    pub async fn evaluate_threat(&self, payload: &[u8]) -> anyhow::Result<ThreatResult> {
        ...
        // Emit telemetry
        self.plasma_bus.emit("plasma.defender.threat.evaluated", ring_strength as f64).await?;
