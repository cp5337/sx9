...
use crate::ann_daemon::{AnnConfig, AnnDaemon, AnnObservation};
...
pub async fn subscribe_tool_results_internal(
    ...
    while let Some(msg) = subscriber.next().await {
        ...
        info!(
            "Received tool result: {} from operator {}",
            tool_result.tool, tool_result.operator_id
        );
        ...
        if let Some(advisory) = ann_daemon.get_advisory(&ctx).await? {
            info!(
                ...
                advisory.recommendation, advisory.confidence
            );
