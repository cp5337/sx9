...
impl TOMLUnicodeCompiler {
    ...
    pub fn compile(toml_content: &str) -> DSLResult<UnicodePlaybook> {
        ...
        if let Some(hash_table) = playbook_table.get("trivariate_hash")
            .and_then(|v| v.as_table()) {
            let trivariate_hash = TrivariateHash {
                sch: hash_table.get("sch")
                    .and_then(|v| v.as_str())
                    .unwrap_or("")
                    .to_string(),
                cuid: hash_table.get("cuid")
                    .and_then(|v| v.as_str())
                    .unwrap_or("")
                    .to_string(),
                uuid: hash_table.get("uuid")
                    .and_then(|v| v.as_str())
                    .unwrap_or("")
                    .to_string(),
            };
        ...
        if let Some(escalation_table) = playbook_table.get("escalation")
            .and_then(|v| v.as_table()) {
            playbook.escalation = EscalationConfig {
                tier_1_wasm: escalation_table.get("tier_1_wasm")
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string()),
                tier_2_microkernel: escalation_table.get("tier_2_microkernel")
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string()),
                tier_3_kernel_crate: escalation_table.get("tier_3_kernel_crate")
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string()),
                tier_4_multi_crates: escalation_table.get("tier_4_multi_crates")
                    .and_then(|v| v.as_array())
                    .map(|arr| arr.iter()
                        .filter_map(|v| v.as_str().map(|s| s.to_string()))
                        .collect()),
                tier_5_containers: escalation_table.get("tier_5_containers")
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string()),
                tier_6_firefly: escalation_table.get("tier_6_firefly")
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string()),
                tier_7_orb: escalation_table.get("tier_7_orb")
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string()),
            };
        }
        ...
        if let Some(unicode_table) = playbook_table.get("unicode_assembly")
            .and_then(|v| v.as_table()) {
            playbook.unicode_assembly = UnicodeAssembly {
                primary_trigger: unicode_table.get("primary_trigger")
                    .and_then(|v| v.as_str())
                    .unwrap_or("")
                    .to_string(),
                escalation_triggers: unicode_table.get("escalation_triggers")
                    .and_then(|v| v.as_array())
                    .map(|arr| arr.iter()
                        .filter_map(|v| v.as_str().map(|s| s.to_string()))
                        .collect())
        ...
        // Extract steps
        if let Some(steps_table) = playbook_table.get("steps")
            .and_then(|v| v.as_table()) {
    ...
    fn parse_step(step_key: &str, step_table: &toml::map::Map<String, Value>) -> DSLResult<UnicodePlaybookStep> {
        let name = step_table.get("name")
            .and_then(|v| v.as_str())
            .unwrap_or(step_key)
            .to_string();
        
        let tier_num = step_table.get("tier")
        ...
        let tier = match tier_num {
            ...
            _ => return Err(DSLError::InvalidParameters(format!("Invalid tier: {}", tier_num))),
        };
        
        // Parse Unicode operation
        let unicode_op_str = step_table.get("unicode_op")
            .and_then(|v| v.as_str())
            .unwrap_or("\\u{E800}");
        
        let unicode_op = Self::parse_unicode_char(unicode_op_str)?;
        
        let tool = step_table.get("tool")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());
        
        let target = step_table.get("target")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());
        
        let depends_on = step_table.get("depends_on")
            .and_then(|v| v.as_array())
            .map(|arr| arr.iter()
                .filter_map(|v| v.as_str().map(|s| s.to_string()))
                .collect())
        ...
        if let Some(metadata_table) = step_table.get("metadata")
            .and_then(|v| v.as_table()) {
    ...
    fn parse_unicode_char(s: &str) -> DSLResult<char> {
        ...
        if let Some(hex_start) = s.find("\\u{") {
            let hex_end = s[hex_start + 3..].find('}')
                .ok_or_else(|| DSLError::InvalidParameters(format!("Invalid Unicode format: {}", s)))?;
            ...
            return char::from_u32(code_point)
                .ok_or_else(|| DSLError::InvalidParameters(format!("Invalid Unicode code point: {}", code_point)));
        }
        ...
        Err(DSLError::InvalidParameters(format!("Unrecognized Unicode format: {}", s)))
    }
}
...
"#;
    ...
    fn test_parse_unicode_char() {
        assert_eq!(TOMLUnicodeCompiler::parse_unicode_char("\\u{E800}").unwrap(), '\u{E800}');
        assert_eq!(TOMLUnicodeCompiler::parse_unicode_char("U+E800").unwrap(), '\u{E800}');
    }
}
