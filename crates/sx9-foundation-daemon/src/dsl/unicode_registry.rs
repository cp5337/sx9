...
impl DSLUnicodeRegistry {
    ...
    fn register_dsl_operation(&mut self, dsl_name: &str, unicode_ops: Vec<char>, primary: char) {
        self.dsl_to_unicode.insert(dsl_name.to_string(), unicode_ops.clone());
        self.dsl_primary_unicode.insert(dsl_name.to_string(), primary);
    ...
    pub fn operation_to_unicode(&self, op: &DSLOperation) -> Vec<char> {
        match op {
            DSLOperation::HashTrigger(_) => {
                self.get_unicode_ops("hash_trigger")
                    .cloned()
                    .unwrap_or_else(|| vec!['\u{E100}'])
            }
            DSLOperation::IntelCollection(_) => {
                self.get_unicode_ops("intel_collection")
                    .cloned()
                    .unwrap_or_else(|| vec!['\u{E300}'])
            }
            DSLOperation::PentestSpawn(_) => {
                self.get_unicode_ops("pentest_spawn")
                    .cloned()
                    .unwrap_or_else(|| vec!['\u{E200}'])
            }
            DSLOperation::EphemeralAsset(_) => {
                self.get_unicode_ops("ephemeral_asset")
                    .cloned()
                    .unwrap_or_else(|| vec!['\u{E000}'])
            }
            DSLOperation::NodeInterview(_) => {
                self.get_unicode_ops("node_interview")
                    .cloned()
                    .unwrap_or_else(|| vec!['\u{E300}'])
            }
            DSLOperation::KaliTool(_) => {
                self.get_unicode_ops("kali_tool")
                    .cloned()
                    .unwrap_or_else(|| vec!['\u{E800}'])
            }
            DSLOperation::Workflow(_) => {
                self.get_unicode_ops("workflow")
                    .cloned()
                    .unwrap_or_else(|| vec!['\u{E000}'])
            }
            DSLOperation::Parallel(_) => {
                self.get_unicode_ops("parallel")
                    .cloned()
                    .unwrap_or_else(|| vec!['\u{E000}'])
            }
            DSLOperation::Conditional(_) => {
                self.get_unicode_ops("conditional")
                    .cloned()
                    .unwrap_or_else(|| vec!['\u{E000}'])
            }
        }
    ...
    pub fn operation_to_primary_unicode(&self, op: &DSLOperation) -> char {
        match op {
            DSLOperation::HashTrigger(_) => self.get_primary_unicode("hash_trigger").unwrap_or('\u{E100}'),
            DSLOperation::IntelCollection(_) => self.get_primary_unicode("intel_collection").unwrap_or('\u{E300}'),
            DSLOperation::PentestSpawn(_) => self.get_primary_unicode("pentest_spawn").unwrap_or('\u{E200}'),
            DSLOperation::EphemeralAsset(_) => self.get_primary_unicode("ephemeral_asset").unwrap_or('\u{E000}'),
            DSLOperation::NodeInterview(_) => self.get_primary_unicode("node_interview").unwrap_or('\u{E300}'),
            DSLOperation::KaliTool(_) => self.get_primary_unicode("kali_tool").unwrap_or('\u{E800}'),
            ...
            DSLOperation::Conditional(_) => self.get_primary_unicode("conditional").unwrap_or('\u{E000}'),
        }
    }
}
