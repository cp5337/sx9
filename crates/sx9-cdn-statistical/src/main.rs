...
async fn main() -> Result<()> {
    ...
    info!(
        "🚀 CTAS-7 Statistical Analysis CDN v7.3.1 listening on {}",
        config.bind_addr
    );
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            if let Err(err) = collect_stats(&state).await {
                warn!("⚠️ stats collector error: {err}");
            }
        }
    })
}
...
async fn register_with_port_manager(state: &AppState) -> Result<()> {
    ...
    info!("✅ Registered statistical CDN with port manager");
    Ok(())
}
