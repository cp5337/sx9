...
mod tests {
    ...
    impl TestBus {
        ...
        fn dispatch(&self, cmd: Command) -> DispatchResult {
            ...
            if success { DispatchResult::Ok } else { DispatchResult::BufferFull }
        }
        ...
        fn plasma(&self) -> &PlasmaState { &self.plasma }
        
        fn respond(&self, result: AtlasResult) -> bool { self.results.push(result) }
        fn pop_result(&self) -> Option<AtlasResult> { self.results.pop() }
    }
    ...
    fn test_dispatch_and_pop() {
        ...
        let cmd = Command::new(CommandKind::Ping { seq: 1, timestamp_ns: 0 });
    ...
    fn test_priority_order() {
        ...
        bus.dispatch(Command::new(CommandKind::Ping { seq: 1, timestamp_ns: 0 })); // Normal
    ...
    fn test_sdt_blocking() {
        ...
        let cmd = Command::critical(CommandKind::Ping { seq: 2, timestamp_ns: 0 });
        assert_eq!(bus.dispatch(cmd), DispatchResult::Ok);
        ...
        let cmd = Command::new(CommandKind::Ping { seq: 3, timestamp_ns: 0 });
        assert_eq!(bus.dispatch(cmd), DispatchResult::Ok);
    }
