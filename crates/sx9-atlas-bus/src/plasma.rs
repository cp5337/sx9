...
impl PlasmaState {
    ...
    pub fn resonate(
        ...
        let new_sdt = self.compute_sdt_transition(current_sdt, ring_strength, config);
    ...
    pub fn resonate_poly(
        ...
        self.last_ring_strength
            .store(result.ring_strength.to_bits(), Ordering::Release);
        ...
        let current_sdt = self.sdt_state();
        let new_sdt = self.compute_sdt_transition(current_sdt, result.ring_strength, config);
    ...
    pub fn trigger(&self, tick: u64) -> bool {
        ...
        if current == SdtState::Primed as u8 {
            self.sdt_state
                .store(SdtState::Conducting as u8, Ordering::Release);
            self.last_trigger_tick.store(tick, Ordering::Release);
            self.trigger_count.fetch_add(1, Ordering::AcqRel);
            true
        } else {
    ...
    pub fn latch(&self) {
        ...
        if current == SdtState::Conducting as u8 {
            self.sdt_state
                .store(SdtState::Latched as u8, Ordering::Release);
        }
    }
