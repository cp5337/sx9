...
pub enum WireCommandKind {
    ...
    ConvergenceCheck { entity_id: u32, epsilon: f32, window: u16 },
    BatchHash { count: u32, item_size: u16 },
    TrivariateHash { domain: u8, execution: u8, delta_class: u8 },
    TickSync { tick_id: u64, timestamp_ns: u64 },
    SdtTrigger { gate_id: u32, reason: u16 },
    SdtReset { gate_id: u32 },
    PlasmaUpdate { field_id: u32, delta_angle: u16, entropy: u32, excited: bool },
...
pub enum WireResultKind {
    Path { nodes: Vec<u32>, cost: f32 },
    MatroidRank { rank: u32 },
    Convergence { converged: bool, current_delta: f32, ticks_stable: u16 },
    TrivariateHash { sch: u64, cuid_hi: u64, cuid_lo: u64, uuid_hi: u64, uuid_lo: u64 },
    TickAck { tick_id: u64, drift_ns: i64 },
    SdtState { gate_id: u32, state: u8, last_trigger_tick: u64 },
    PlasmaState { field_id: u32, delta_angle: u16, entropy: u32, excited: bool },
    Pong { seq: u32, request_timestamp_ns: u64, response_timestamp_ns: u64 },
    Stats { commands_processed: u64, avg_latency_ns: u64, queue_depth: u32, uptime_secs: u64 },
    Ack,
...
impl NatsBridge {
    ...
    async fn init_streams(&mut self) -> anyhow::Result<()> {
        ...
        let _ = js.get_or_create_stream(jetstream::stream::Config {
            ...
            subjects: vec![format!("{}.*.trigger", subjects::SDT_TRIGGER), format!("{}.*.state", subjects::SDT_STATE)],
            max_messages: 50_000,
    ...
    pub async fn subscribe_sdt(&self, gate_id: Option<u32>) -> anyhow::Result<async_nats::Subscriber> {
...
pub async fn broadcast_tick(client: &Client, tick_id: u64, instance_id: &str) -> anyhow::Result<()> {
    let msg = TickSync {
        tick_id,
        timestamp_ns: std::time::SystemTime::now()
...
mod tests {
    ...
    fn test_wire_command_serialization() {
        let cmd = WireCommand {
            kind: WireCommandKind::Dijkstra { src: 1, dst: 2, max_hops: 5 },
            sch_hash: 0xDEADBEEF,
            tick_id: 42,
            priority: 1,
            request_id: 123,
