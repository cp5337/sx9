# RFC-9016 — Dual-Trivariate PTCC Integration & Delta-Angle Expansion
...
**Noise Score Formula:**
```
noise_score = (normalized_angle × 0.4) + (entropy_drift × 0.3) + (semantic_drift × 0.3)
```

Where `normalized_angle = min(|delta_angle| / 180.0, 1.0)`
...
pub fn compute_delta_angle(&self, ctx1: &ContextFrame, ctx2: &ContextFrame) -> f32 {
    ...
    // Magnitude as degrees
    let magnitude = (delta_timestamp.powi(2) + 
                    delta_agent.powi(2) + 
                    delta_lineage.powi(2) + 
                    delta_angle_rad.powi(2)).sqrt();
    
    magnitude.to_degrees().min(180.0)
}
```
...
- 0x000-0xFFF: 0.0° to 180.0° in 0.044° increments
